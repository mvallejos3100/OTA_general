...
class OTAUpdater:
    ...
    def __init__(self, github_repo, github_src_dir='', module='', main_dir='main', new_version_dir='siguiente', secrets_file=None, headers={}):
    ...
    def check_for_update_to_install_during_next_reboot(self) -> bool:
        ...
        if latest_version > current_version:
            print('Nueva versión disponible, se descargará e instalará en el siguiente reinicio')
    ...
    def install_update_if_available_after_boot(self, ssid, password) -> bool:
        ...
        if self.new_version_dir in os.listdir(self.module):
            if '.version' in os.listdir(self.modulepath(self.new_version_dir)):
                ...
                print('Actualización encontrada: ', latest_version)
    ...
    def install_update_if_available(self) -> bool:
        ...
        (current_version, latest_version) = self._check_for_new_version()
        if latest_version > current_version:
            print('Actualizando a la versión {}...'.format(latest_version))
            self._create_new_version_file(latest_version)
    ...
    def _download_new_version(self, version):
        print('Descargando la nueva versión {}'.format(version))
        self._download_all_files(version)
        print('Versión {} descargada a {}'.format(version, self.modulepath(self.new_version_dir)))
    ...
    def _download_all_files(self, version, sub_dir=''):
        ...
        for file in file_list.json():
            ...
            if file['type'] == 'file':
                gitPath = file['path']
                print('\tDescargando: ', gitPath, 'to', path)
    ...
    def _copy_secrets_file(self):
        if self.secrets_file:
            ...
            print('Copiando archivos secretos de {} a {}'.format(fromPath, toPath))
    ...
    def _install_new_version(self):
        ...
        print('Actualización instalada, por favor reinicie ahora')
